if vim.g.vscode then
  ...
  vim.keymap.set('x', '<leader>e', 'y:%s/<C-r><C-r>"//g')
  vim.keymap.set('n', '<leader>e', 'yiw:%s/<C-r><C-r>"//g')
  ...
  -- vim.keymap.set('n', 'h', '4h')
  -- -- Map 'k' to '4k' in normal mode
  ...
  require('lazy').setup {
    ...
    { 'numToStr/Comment.nvim', opts = {} },
    {
      ...
      -- Customize highlight colors for quick-scope, which is must for working as VSCode extension.
      config = function()
        vim.cmd [[
          highlight QuickScopePrimary guifg='#00ffff' gui=underline ctermfg=155 cterm=underline
          highlight QuickScopeSecondary guifg='#ff69b4' gui=underline ctermfg=81 cterm=underline
        ]]
      end,
    },
    {
    ...
    { -- Collection of various small independent plugins/modules
      ...
      config = function()
        ...
        -- - sa"   - [S]urround [A]dd ["]double quotes, when in "v" mode.
  ...
  -- Map <leader>cc to print the full directory path of the current file
  ...
  if username == 'enantiomer-s' then
    hostname = os.getenv 'HOSTNAME'
    if hostname == 'Central-Dogma' then
      vim.g.have_nerd_font = true
    else
      vim.g.have_nerd_font = false
    end
  elseif username == 'huang' then
    vim.g.have_nerd_font = true
  end
  ...
  vim.api.nvim_create_autocmd({ 'BufRead', 'BufNewFile' }, {
    pattern = '*.def',
    callback = function()
      vim.bo.filetype = 'sh' -- Use shell syntax highlighting as a fallback
    end,
  })
  ...
  require('lazy').setup({
    ...
    },
    {
      ...
      init = function()
        ...
        -- Define custom mappings (optional)
        vim.g.VM_maps = {
            ["Find Under"] = "<C-n>",
    ...
    -- NOTE: Bullets works, but not necessary.
    -- {
    ...
    --       mapping = { 'mz' }, -- Set 'mz' as your escape sequence
    --       timeout = 300, -- Customize the timeout length
    --     }
    --   end,
    -- },
    {
    ...
    { -- Autoformat
      ...
      opts = {
        ...
        --   local disable_filetypes = { c = true, cpp = true, html = true, markdown = true, md = true, python = true, py = true }
    ...
    -- Highlight todo, notes, etc in comments
    {
      ...
      opts = {
        signs = false,
        highlight = {
          comments_only = false,  -- Set to false if you want to highlight outside comments
          pattern = {
            -- [[.*<(KEYWORDS)\s*:]],
            [[.*<(KEYWORDS)\s*]],
            -- [[.*<(!KEYWORDS])\s*]],
        ...
        keywords = {
          -- Add a custom keyword for "> [!"
          SUCCESS = {
            icon = " ",  -- You can choose any icon or leave it empty
            color = "hint",  -- Choose a color (hint, info, warning, error)
            alt = { 'DONE' },  -- Define the pattern as an alternative keyword
          },
        },
      }
    },
    ...
    { -- Highlight, edit, and navigate code
      ...
      opts = {
        ensure_installed = { 'bash', 'c', 'diff', 'html', 'lua', 'luadoc', 'markdown', 'markdown_inline', 'query', 'dockerfile', 'vim', 'vimdoc' },
    ...
    -- { import = 'custom.plugins.molten' },
    { import = 'custom.plugins.chatgpt' },
    { import = 'custom.plugins.vim-tmux-navigator' },
    { import = 'custom.plugins.outline' },
